...
const CommunityMessages = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const senderId = 2; // Operational Manager user ID
      const senderName = localStorage.getItem('userEmail') || 'Operations Manager';
      const senderType = 'STAFF'; // Try STAFF first to test if the API works

      // Use the same format as receptionist but with trimmed values
      const messageData = {
      ...
      console.log('Sender info:', { senderId, senderName, senderType });

      await messagesAPI.createMessage(messageData, senderId, senderName, senderType);
  ...
  const handleReply = async (parentMessageId) => {
    ...
    try {
      ...
      const replyData = {
        ...
        parentMessageId,
        subject: 'Reply' // Add subject for replies
      };

      console.log('Sending reply data:', replyData);
      console.log('Reply sender info:', { senderId, senderName, senderType });
