...
export const createReservation = async (reservationData) => {
  try {
    // Extract numeric guest ID from formats like "GUEST056970" -> "56970"
    const normalizeGuestId = (gid) => {
      if (!gid) return null;
      const gidStr = String(gid);
      // Extract only numeric characters from strings like "GUEST056970"
      const match = gidStr.match(/\d+/);
      return match ? match[0] : gidStr;
    };
    
    const payload = {
      guestId: normalizeGuestId(reservationData.guestId),
...
export const getGuestReservations = async (guestId) => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/reservations/guest/${numericGuestId}`,
      { method: "GET", headers: getHeaders() }
    );
...
export const searchReservations = async (criteria) => {
  try {
    ...
    if (criteria.guestId) params.append('guestId', normalizeGuestId(criteria.guestId));
...
export const getGuestHolds = async (guestId) => {
  try {
    ...
    const numericGuestId = normalizeGuestId(guestId);
    const response = await fetch(
      `${API_BASE_URL}/api/holds/guest/${numericGuestId}`,
      { method: "GET", headers: getHeaders() }
    );
